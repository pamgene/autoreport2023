...
Kinases phosphorylate multiple phosphosites on the PamChip. To address which kinases are responsible for the phosphorylation differences between conditions, we perform Upstream Kinase Analysis (UKA). The UKA predicts potential kinases by examining changes in their associated phosphosite sets. The phosphosite sets are derived from publicly available databases of kinase-protein-phosphosite relationships. For the details of the algorithm, explanation of the scores and further visualizations (kinome trees), refer to Supplement 2.4.

### Top kinases table

The table below shows the top 10 kinases using the threshold `r if(params$csUKA) "Final Score" else "Median Final Score"` > `r params$fscore_thr` and `r if(params$csUKA) "Specificity Score" else "Mean Specificity Score"` > `r params$spec_thr`.
...
The Kinase dotplots below show the specificity (`r if(params$csUKA) "Specificity Score" else "Mean Specificity Score"`) and Fold Change (`r if(params$csUKA) "Kinase Statistic" else "Median Kinase Statistic"`) of the significant kinases in different comparisons. Significance threshold is set to `r if(params$csUKA) "Final Score" else "Median Final Score"`  > `r params$fscore_thr` and `r if(params$csUKA) "Specificity Score" else "Mean Specificity Score"` > `r params$spec_thr`. Bigger dots mean higher specificity. The kinases are clustered by families.

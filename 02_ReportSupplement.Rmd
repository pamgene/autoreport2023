...
1. Affine transformation – centers and scales the data. This results in comparisons based on relative changes - deviations from the overal mean - rather than absolute values. For example, in a sample with higher overal kinase activity than control, VSN transformation results in kinases with above- and below-average activity. In a volcano-plot this shows significant peptides with both negative and positive fold change, with the most positive indicating key drivers.\n
...
1. Phosphosite level data: we suggest to combine the PTK and STK significant results. Use the proteins mapped to phosphosites (in the “_enriched” files). Since multiple phosphosites can map to one protein, use their mean fold change. (Significant phosphosites: p value<0.05; or set your own more/ less stringent cut-off). 

2. Kinase level data: We suggest to combine the PTK and STK significant kinases (`r if(params$csUKA) "Final score" else "Median Final score"` >1.3; or set your own more/ less stringent cut-off). Effect direction and size: Use `r if(params$csUKA) "Kinase Statistic" else "Median Kinase Statistic"`. 

3. Combine (1) and (2) into one table, with a column for ID (e.g. Gene name or UniprotID), and a column for the LFC values (from (1)=LFC; From (2)=`r if(params$csUKA) "Kinase Statistic" else "Median Kinase Statistic"`) and save as a txt file. When there is an overlap between kinases and the proteins mapped to phosphosites, we suggest to use kinase data. 
...
| [Lomics](https://lomics.ai/)                       | Research question | AI-based tool that gives potential pathways and pathway members upon a question.  |
...
The [Dark Kinome](https://darkkinome.org/data) provides information about not-well-studied kinases (network, expression, pathways, assays to study).
...
1.	PamDx analysis identifies activated kinases in the lysate by detecting phosphorylation of their targets (phosphosites) on the PamChip. It is possible that targets are not present in the lysate in their native form, but the predicted kinase is present. 
...
Sites such as Uniprot and PhosphoSitePlus can help in choosing the specific phosphosites or PamDx can help identifying such phosphosites on request, as an additional service.

4.	If the approach to validate a kinase is not possible, validation of downstream phosphosites activated by the kinase can be performed. PamDx can help identifying such phosphosites on request.

You can find more information [here](https://pamdx.com/wp-content/uploads/2020/09/Flyer-Confirming-PamChip-Data.pdf).

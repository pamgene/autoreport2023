...
cat("* **Kinome Tree**, useful to group the kinases into phylogenetic families. The Coral Trees show all kinases above Kinase Score > ", params$fscore_thr, ". You can also generate and fine tune your own [Coral Trees](http://phanstiel-lab.med.unc.edu/CORAL/).")
```

Results are provided as Excel (csv) files (Data tables/UKA_PTK or UKA_STK). The column Median Final Score (= Kinase Score) is used for the ranking, thus enabling selection of kinases above the default threshold (Kinase Score > 1.3) or a stricter threshold.
...
2. Select top kinase(s) based on:
    + Ranking order
    + Threshold cut-off (depends on research question; guideline is to minimally use Median Final score > 1.3 which is equivalent to p-value < 0.05.)
...
1. Phosphosite level data: we suggest to combine the PTK and STK significant results. Use the proteins mapped to phosphosites (in the “_enriched” files). Since multiple phosphosites can map to one protein, we suggest to use the mean fold change. (Significant phosphosites: p value<0.05; or set your own more/ less stringent cut-off). 

2. Kinase level data: We suggest to combine the PTK and STK significant kinases (Median Final score >1.3; or set your own more/ less stringent cut-off). Effect direction and size: Use Median Kinase Statistic. 

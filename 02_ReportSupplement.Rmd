...
The UKA algorithm returns the followings:
...
```{r include=any(c("table", "splotf", "splots", "tree") %in% params), results='asis'}
cat("The UKA algorithm generates the following plots:")
```

```{r include="table" %in% params, results='asis'}
cat("* **Kinase Score Table**, useful to identify the top kinases. 
The Score Table shows the top 10 kinases regardless of the Kinase Score threshold.")

```

```{r include= ("splotf" %in% params | "splots" %in% params), results='asis'}
cat("* **Kinase Score Plot**, useful to identify the effect size and direction of the top 20 kinases. 
Plots are ordered by the Kinase score and colored by Specificity Score or kinase family.")
```

```{r include= "tree" %in% params, results='asis'}

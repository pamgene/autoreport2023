...
```{r analysis-workflow, message=FALSE, fig.align='center', fig.cap='Data analysis workflow.', fig.width=6.3, fig.height=2.68}
...
if (nrow(qc_files) > 0) {
  ...
  show_variability_section <- should_show_variability_section(qc_files, qc_parsed$variability_table)
} else {
  qc_assay_types <- ""
...
if (nrow(qc_files) > 0 && any(qc_files$Replicate_Type == "TR")) {
  ...
  cat("* Data variability (i.e., the median within-condition peptide signal standard deviation (SD), based on technical replicates).\n\n")
...
if (show_variability_section) {
  ...
  cat("Alongside the QC flag, data variability is reported across three tiers (low/medium/high, Table 3). Lower variation enables detection of smaller effect sizes from the covariates of interest. With high variability a larger sample size may be required to detect differentially active phosphosites or kinases reliably. The Data Variability Indicator uses median peptide standard deviation (SD) per condition (e.g. test, control). Table 4 reports the range (minimum – maximum) of this median SD across all conditions and normalizations in the study.")
}
```

```{r variability ref table}
if (show_variability_section) {
  render_variability_ref_table(qc_parsed$variability_table)
}
```

```{r variability table}
if (show_variability_section) {
  render_variability_table(qc_parsed$variability_table)
}
```

...
`r if (!"Limma" %in% stats_files$Stats) {"\\begin{comment}"}`
* LIMMA (Linear Models for Micro Arrays): LIMMA, instead of analyzing each peptide in isolation, uses information from all the peptides and conditions in the experiment to improve its analysis. Applying LIMMA leads to results with increased statistical power over individual testing per peptide. 
`r if (!"Limma" %in% stats_files$Stats) {"\\end{comment}"}`
...
```{r limmavolcanos, fig.cap = paste("Limma Volcano Plots", if(nrow(stats_files) > 0) {stats_files %>% filter(Stats == "Limma") %>% pull(Group)}), dpi=150, fig.height=3 * length(assay_types)}
